...
TEST_CASE("Test cross product", "[cross2d]")
{
  ...
  CHECK_THAT(v12, WithinAbs(-20.02, TOLERANCE));

  Vector2D v3{9.2, 4.6};
  Vector2D v4{-3.4, -6.7};

  double v34 = cross2d(v3, v4);
  CHECK_THAT(v34, WithinAbs(-46.0, TOLERANCE));
}
...
TEST_CASE("Test dot product", "[dot2d]")
{
  ...
  CHECK_THAT(v12, WithinAbs(-20.67, TOLERANCE));

  Vector2D v3{-2.3, -5.6};
  Vector2D v4{-1.1, -4.3};

  double v34 = dot2d(v3, v4);
  CHECK_THAT(v34, WithinAbs(26.61, TOLERANCE));
}
...
TEST_CASE("Test can intersect", "[can_intersect]")
{
  ...
  CHECK(can_intersect(theta1, obs1));
  CHECK_FALSE(can_intersect(theta2, obs1));
}
...
TEST_CASE("Test find distance", "[distance]") /// Allen Liu
{
  ...
  CHECK_THAT(find_distance(theta1, obs1), WithinAbs(sqrt(3.0), TOLERANCE));

  Obstacle obs2{1.0, 1.0, 1.0};
  double theta2 = PI / 2.0;

  CHECK_THAT(find_distance(theta2, obs2), WithinAbs(1.0, TOLERANCE));
}
}
